...
class TwitterStreamImpl extends TwitterBaseImpl implements TwitterStream {
     ...
     * @throws TwitterException when TwitterRepository service or network is unavailable
     * @see twitter4j.StatusStream
     * @see <a href="https://dev.twitter.com/docs/streaming-api/methods">Streaming API Methods | TwitterRepository Developers</a>
    ...
    abstract class TwitterStreamConsumer extends Thread {
        ...
        TwitterStreamConsumer(Mode mode) {
            super();
            this.mode = mode;
            NAME = format("TwitterRepository Stream consumer / %s [%s]", conf.getStreamThreadName(), ++count);
            updateListeners();
            ...
            while (!closed) {
                try {
                    ...
                    if (!closed) {
                        ...
                        if (!closed) {
                            // wait for a moment not to overload TwitterRepository API
